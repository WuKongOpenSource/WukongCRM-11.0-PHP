...
class Trash extends ApiCommon
{
    ...
    **/    
    ...
    public function delete()
    {
        ...
        # 权限判断
        $workId = Db::name('task')->where(['ishidden' => 1,'task_id' => $param['task_id']])->value('work_id');
        if (!empty($workId) && !$this->checkWorkOperationAuth('cleanTask', $workId, $this->userInfo['id'])) {
    ...
    public function recover()
    {
        ...
        # 权限判断
        $workId = Db::name('task')->where(['ishidden' => 1,'task_id' => $param['task_id']])->value('work_id');
        if (!empty($workId) && !$this->checkWorkOperationAuth('restoreTask', $workId, $this->userInfo['id'])) {
            header('Content-Type:application/json; charset=utf-8');
            exit(json_encode(['code' => 102, 'error' => '无权操作！']));
        }
