...
class taskclass extends ApiCommon
{
    ...
    **/    
    ...
    public function save()
    {
        ...
        # 权限判断
        if (!empty($param['work_id']) && !$this->checkWorkOperationAuth('saveTaskClass', $param['work_id'], $userId)) {
...
//        }
    ...
    public function rename()
    {
		...
		$param['create_user_id'] = $userInfo['id'];
        # 权限判断
        if (!empty($param['work_id']) && !$this->checkWorkOperationAuth('updateTaskClass', $param['work_id'], $userInfo['id'])) {
            header('Content-Type:application/json; charset=utf-8');
            exit(json_encode(['code' => 102, 'error' => '无权操作！']));
        }
...
//        }
    ...
    public function delete()
    {
		...
		$param['create_user_id'] = $userInfo['id'];
        # 权限判断
        if (!empty($param['work_id']) && !$this->checkWorkOperationAuth('deleteTaskClass', $param['work_id'], $userInfo['id'])) {
            header('Content-Type:application/json; charset=utf-8');
            exit(json_encode(['code' => 102, 'error' => '无权操作！']));
        }
